...
const ExploreCard = ({ id, imgUrl, title, index, active, handleClick }) => (
  <motion.div
    ...
    className={`relative ${active === id ? 'lg:flex-[3.5] flex-[10]' : 'lg:flex-[.5] flex-[2]'} 
                flex items-center justify-center min-w-[170px] h-[700px] transition-[flex] duration-[.7s] ease-out-flex
                cursor-pointer`}
    onClick={() => handleClick(id)}
  >
    <img src={imgUrl} alt={title} className="abolute w-full h-full object-cover rounded-[24px]" />
    {
      active !== id ? (
        <h3 className="fomr-semibold sm:text-[26px] text-[18px] text-white 
